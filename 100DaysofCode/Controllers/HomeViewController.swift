    ...
    func animateTrackShadow() {
        DispatchQueue.main.async {
            UIView.animate(withDuration: 2, animations: {
                self.trackLayer.shadowOpacity = 0
            }) { (state) in
                UIView.animate(withDuration: 1, animations: {
                    self.trackLayer.shadowOpacity = 1
                }) { (state) in
                    UIView.animate(withDuration: 1, animations: {
                        self.trackLayer.shadowOpacity = 0
...
//        UIView.animateKeyframes(withDuration: 4, delay: 0, options: .autoreverse, animations: {
            ...
            NetworkingProvider.returnCommitData(completion: { (hasCommited, streak, nodes) in
                FirebaseController.instance.returnUserStreak(completion: { (lastStreak) in
                    if streak > lastStreak {
                        ...
                        self.newCounterLabel.text = String(lastStreak)
